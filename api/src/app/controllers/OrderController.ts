...
class OrderController {
  static async getOrders(req: IOrderRequest, res: Response) {
    const orders: any = await OrderServices.getOrders(req.body);
  ...
  static async getSingleOrder(req: IOrderRequest, res: Response) {
    const order: any = await OrderServices.getSingleOrders({ id: req?.params?.orderId });
    if (order && !order?.statusCode) {
      res.status(StatusCodes.OK).json(formatPayload(true, order));
    } else {
      res.status(StatusCodes.BAD_REQUEST).json(formatPayload(false, 'Opps something went wrong'));
    }
  }
